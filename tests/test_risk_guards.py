...
from aqtc.financial_core.signals import PositionSignal, TradingSignal, cap_signal
...
def sample_signal(gross=6.0):
    return TradingSignal(
        ...
        model="test",
        positions=[
            PositionSignal("A", gross / 2, 1.0, 1.0),
            PositionSignal("B", gross / 2, 1.0, 1.0),
        ],
...
def test_live_trading_disabled_by_default():
    assessment = RiskGuard(RiskPolicy(live_trading=False)).assess(
        sample_signal(1.0), live_requested=True
    )
    assert assessment.allowed is False
