# AQTC Pitch

## Scientific integrity first

AQTC **rejects** the 2019+ ensemble (Sharpe **-0.544**) before promoting HGAT+ES v4 (Sharpe **3.255**, **5/5** folds). Falsification is the product — not a footnote.

**vs SOLVENT:** SOLVENT sells research briefs; AQTC sells evolved alpha proven with 5-fold walkforward.

**Stripe proof:** test PaymentIntent **succeeded** — `docs/proof/stripe_test_paymentintent_redacted.json`

### Slide numbers
- Accepted: Sharpe **3.255**, 5/5 folds, MaxDD **0.032**
- Rejected: Sharpe **-0.544**, MaxDD **0.486**
- Business: spend $2, earn $19, **net $17**



## One-liner

**No es prompt trading. Es alpha evolucionado por Financial Lab, validado por walkforward, operado por Hermes como una micro-compañía cuantitativa auditable.**

## Catch

**From evolved alpha to invoice.**

## Subtitle

HGAT+ES alpha, Hermes operations, Stripe revenue.

## Problem

Most AI trading demos are prompt → signal → chart. They hide bad backtests, skip business operations, and can't explain where alpha came from.

## Solution

AQTC is an autonomous quant company that:

1. **Proves alpha origin** — frozen Financial Lab HGAT+ES v4 walkforward evidence (Sharpe 3.255, 5 folds, 100% positive); [why this architecture](WHY_HGAT_ES.md)
2. **Falsifies bad strategies** — explicitly rejects 2019+ ensemble (Sharpe -0.544, MaxDD 0.486)
3. **Runs operations** — Stripe spend/earn, NemoClaw-compatible policy boundary, paper MockBroker, customer reports
4. **Integrates with Hermes** — CLI, MCP tools, FastAPI dashboard

## Hackathon alignment

| Sponsor | Integration |
|---------|-------------|
| NVIDIA | Nemotron regime summarizer (mock default, live opt-in) |
| Stripe | Skills-style ledger: spend $2 data, earn $19 report, net $17 |
| Nous Research | Hermes agent operations via MCP |

## Demo in 30 seconds

```bash
aqtc provenance --json   # HGAT+ES evidence
aqtc demo                # full business cycle, net $17
make serve               # dashboard with provenance + falsification cards
```

## Key metrics

- Accepted: mean Sharpe **3.255**, mean MaxDD **0.032**, 19D genotype φ
- Rejected: Sharpe **-0.544**, MaxDD **0.486**
- Business: spend $2, earn $19, **net $17**

## Safety

- Live trading disabled by default
- ES is provenance, not runtime spectacle
- Human approval for spend above threshold

## Ask

Vote for a demo that shows **real quant operations** — not another black-box trading bot.
