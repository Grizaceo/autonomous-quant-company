...
render_scene () {
  local bg="$1" audio="$2" srt="$3" top_tag="$4" out="$5" duration="$6"
  python3 "$SCRIPTS/render_scene.py" \
    --bg "$bg" --audio "$audio" --srt "$srt" --top-tag "$top_tag" --out "$out" \
    --clips-dir "$CLIPS" --duration "$duration"
}
...
render_scene "$CLIPS/scene1_bg.mp4" "$VOICE/scene1_hook.mp3" "$CLIPS/scene1.srt" \
  "From evolved alpha to invoice" "$CLIPS/scene1.mp4" "$D1"
...
render_scene "$CLIPS/scene1b_bg.mp4" "$VOICE/scene1b_clarify.mp3" "$CLIPS/scene1b.srt" \
  "What is Financial Lab?" "$CLIPS/scene1b.mp4" "$D1B"

echo "[2] HGAT+ES provenance slide"
render_bg_frame "$CLIPS/scene2_hgat_es.png" "$D2" "$CLIPS/scene2_bg.mp4"
render_scene "$CLIPS/scene2_bg.mp4" "$VOICE/scene2_origin.mp3" "$CLIPS/scene2.srt" \
  "Financial Lab: Heterogeneous Graph Attention + Evolution Strategies" "$CLIPS/scene2.mp4" "$D2"

echo "[3] falsification slide (rejected vs accepted)"
render_bg_frame "$CLIPS/scene3_falsification.png" "$D3" "$CLIPS/scene3_bg.mp4"
render_scene "$CLIPS/scene3_bg.mp4" "$VOICE/scene3_falsify.mp3" "$CLIPS/scene3.srt" \
  "Falsification: 2019+ ensemble rejected" "$CLIPS/scene3.mp4" "$D3"

echo "[4] business loop slide (policy + Nemotron + ledger)"
render_bg_frame "$CLIPS/scene4_business_loop.png" "$D4" "$CLIPS/scene4_bg.mp4"
render_scene "$CLIPS/scene4_bg.mp4" "$VOICE/scene4_business.mp3" "$CLIPS/scene4.srt" \
  "Business loop: policy gate + Nemotron + Stripe ledger" "$CLIPS/scene4.mp4" "$D4"

echo "[5] closing slide"
render_bg_frame "$CLIPS/scene5_close.png" "$D5" "$CLIPS/scene5_bg.mp4"
render_scene "$CLIPS/scene5_bg.mp4" "$VOICE/scene5_close.mp3" "$CLIPS/scene5.srt" \
  "From evolved alpha to invoice." "$CLIPS/scene5.mp4" "$D5"

echo "[concat with crossfades]"
python3 "$SCRIPTS/concat_with_transitions.py" --out "$OUT" --xfade 0.35 \
  "$CLIPS/scene1.mp4" "$CLIPS/scene1b.mp4" "$CLIPS/scene2.mp4" \
  "$CLIPS/scene3.mp4" "$CLIPS/scene4.mp4" "$CLIPS/scene5.mp4"
